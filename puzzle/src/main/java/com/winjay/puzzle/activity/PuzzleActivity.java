...
import com.winjay.puzzle.adapter.GridItemsAdapter;
import com.winjay.puzzle.bean.ItemBean;
import com.winjay.puzzle.util.GameUtil;
import com.winjay.puzzle.util.ImagesUtil;
...
public class PuzzleActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        int id = v.getId();// 返回按钮点击事件
        if (id == R.id.btn_puzzle_main_back) {
            finish();
            // 显示原图按钮点击事件
        } else if (id == R.id.btn_puzzle_main_img) {
            ...
            // 重置按钮点击事件
        } else if (id == R.id.btn_puzzle_main_restart) {
            cleanConfig();
            generateGame();
            recreateData();
            // 通知GridView更改UI
    ...
    private void cleanConfig() {
        ...
        if (mPicPath != null) {
            // 删除照片
            File file = new File(PuzzleMainActivity.TEMP_IMAGE_PATH);
            if (file.exists()) {
                file.delete();
            }
        }
