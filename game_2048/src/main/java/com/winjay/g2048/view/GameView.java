...
public class GameView extends GridLayout implements OnTouchListener {
    ...
    public GameView(Context context) {
        super(context);
        mTarget = Game2048Activity.mSp.getInt(Game2048Activity.KEY_GAME_GOAL, 2048);
        initGameMatrix();
    }
    ...
    public void startGame() {
        initGameMatrix();
        initGameView(Game2048Activity.mItemSize);
    }
    ...
    public void revertGame() {
        ...
        if (sum != 0) {
            Game2048Activity.getGameActivity().setScore(mScoreHistory, 0);
            Game2048Activity.SCROE = mScoreHistory;
            for (int i = 0; i < mGameLines; i++) {
                for (int j = 0; j < mGameLines; j++) {
    ...
    private void initGameMatrix() {
        ...
        removeAllViews();
        mScoreHistory = 0;
        Game2048Activity.SCROE = 0;
        Game2048Activity.mGameLines = Game2048Activity.mSp.getInt(Game2048Activity.KEY_GAME_LINES, 4);
        mGameLines = Game2048Activity.mGameLines;
        ...
        mHighScore = Game2048Activity.mSp.getInt(Game2048Activity.KEY_HIGH_SCROE, 0);
        ...
        Game2048Activity.mItemSize = metrics.widthPixels / Game2048Activity.mGameLines;
        initGameView(Game2048Activity.mItemSize);
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isMoved()) {
                    addRandomNum();
                    // 修改显示分数
                    Game2048Activity.getGameActivity().setScore(Game2048Activity.SCROE, 0);
                }
                checkCompleted();
                break;
            default:
    ...
    private void saveHistoryMatrix() {
        mScoreHistory = Game2048Activity.SCROE;
        for (int i = 0; i < mGameLines; i++) {
            for (int j = 0; j < mGameLines; j++) {
    ...
    private void checkCompleted() {
        ...
        if (result == 0) {
            if (Game2048Activity.SCROE > mHighScore) {
                Editor editor = Game2048Activity.mSp.edit();
                editor.putInt(Game2048Activity.KEY_HIGH_SCROE, Game2048Activity.SCROE);
                editor.apply();
                Game2048Activity.getGameActivity().setScore(Game2048Activity.SCROE, 1);
                Game2048Activity.SCROE = 0;
            }
            AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
            builder.setTitle("Game2048Activity Over")
                    ...
                    .setNegativeButton("Continue",
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface arg0,
                                    ...
                                    Editor editor = Game2048Activity.mSp.edit();
                                    if (mTarget == 1024) {
                                        editor.putInt(Game2048Activity.KEY_GAME_GOAL, 2048);
                                        mTarget = 2048;
                                        Game2048Activity.getGameActivity().setGoal(2048);
                                    } else if (mTarget == 2048) {
                                        editor.putInt(Game2048Activity.KEY_GAME_GOAL, 4096);
                                        mTarget = 4096;
                                        Game2048Activity.getGameActivity().setGoal(4096);
                                    } else {
                                        editor.putInt(Game2048Activity.KEY_GAME_GOAL, 4096);
                                        mTarget = 4096;
                                        Game2048Activity.getGameActivity().setGoal(4096);
                ...
                if (currentNum != 0) {
                    if (mKeyItemNum == -1) {
                        ...
                        if (mKeyItemNum == currentNum) {
                            ...
                            Game2048Activity.SCROE += mKeyItemNum * 2;
                            mKeyItemNum = -1;
                        } else {
