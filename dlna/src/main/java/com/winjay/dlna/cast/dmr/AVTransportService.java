...
public class AVTransportService extends AbstractAVTransportService {
    ...
    public void setAVTransportURI(UnsignedIntegerFourBytes instanceId,
        ...
        LogUtil.d(TAG, "currentURI=" + currentURI);
        LogUtil.d(TAG, "currentURIMetaData=" + currentURIMetaData);
        URI uri;
        try {
        ...
        if (currentURI.startsWith("http:")) {
            try {
                ...
                LogUtil.w(TAG, "RESOURCE_NOT_FOUND: exception=" + ex.getMessage());
                throw new AVTransportException(AVTransportErrorCode.RESOURCE_NOT_FOUND, ex.getMessage());
            }
        } else if (!currentURI.startsWith("file:")) {
            LogUtil.w(TAG, "INVALID_ARGS, Only HTTP and file: resource identifiers are supported.");
...
//            final ClockTime ct = ClockTime.fromSeconds(ModelUtil.fromTimeString(target));
            ...
            LogUtil.d(TAG, "### " + unit + " target: " + target + "  pos: " + pos);
