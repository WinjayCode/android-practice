...
public class GenerateXml {
    ...
    protected void populateDescMetadata(Element descElement, DescMeta descMeta) {
        if (descMeta.getMetadata() instanceof Document) {
            ...
            LogUtil.w(TAG, "Unknown desc metadata content, please override populateDescMetadata(): " + descMeta.getMetadata());
        }
    }
    ...
    protected void generateItem(Item item, Document descriptor, Element parent) {
        ...
        if (title == null) {
            LogUtil.w(TAG, "Missing 'dc:title' element for item: " + item.getId());
            title = UNKNOWN_TITLE;
        }
