...
import com.winjay.dlna.cast.activity.ContentActivity;
...
public class ContentBrowseActionCallback extends Browse {
    ...
    private static final String TAG = "ContentBrowseActionCallback";
    ...
    public void received(final ActionInvocation actionInvocation, final DIDLContent didl) {
        LogUtil.d(TAG, "Received browse action DIDL descriptor, creating tree nodes");
            ...
            public void run() {
                try {
                    ...
                    for (Container childContainer : didl.getContainers()) {
                        LogUtil.d(TAG, "add child container " + childContainer.getTitle());
                        list.add(new ContentItem(childContainer, service));
                    }
                    // Now items
                    for (Item childItem : didl.getItems()) {
                        LogUtil.d(TAG, "add child item" + childItem.getTitle());
                        list.add(new ContentItem(childItem, service));
                    }
                } catch (Exception ex) {
                    LogUtil.e(TAG, "Creating DIDL tree nodes failed: " + ex);
                ...
                while (iterator.hasNext()) {
                    ...
                    if ((contentItem.getItem().getTitle().toString() != null)
                        ...
                        if ((list.size() != 0)
                            ...
                            if (((Res) list.get(0)).getProtocolInfo().getContentFormat()
                            ...
                            } else if (((Res) list.get(0)).getProtocolInfo().getContentFormat()
                                    .substring(0, ((Res) list.get(0)).getProtocolInfo().getContentFormat().indexOf("/"))
                                    .equals("audio")) {
