...
import com.winjay.dlna.cast.dmp.GPlayer.MediaListener;
...
//        GPlayer.setMediaListener(new GstMediaListener());
        ExoPlayerActivity.setMediaListener(new GstMediaListener());
    ...
    synchronized public void setVolume(double volume) {
        LogUtil.d(TAG, "setVolume " + volume);
    ...
    synchronized public void setMute(boolean desiredMute) {
        if (desiredMute && getVolume() > 0) {
            LogUtil.d(TAG, "Switching mute ON");
            setVolume(0);
        } else if (!desiredMute && getVolume() == 0) {
            LogUtil.d(TAG, "Switching mute OFF, restoring: " + storedVolume);
...
*/
    synchronized protected void transportStateChanged(TransportState newState) {
        ...
        LogUtil.d(TAG, "Current state is: " + currentTransportState + ", changing to new state: " + newState);
    ...
    protected class GstMediaListener implements ExoPlayerActivity.MediaListener {
        public void pause() {
            LogUtil.d(TAG);
        ...
        public void endOfMedia() {
            LogUtil.d(TAG, "End Of Media event received, stopping media player backend");
            transportStateChanged(TransportState.NO_MEDIA_PRESENT);
            //GstMediaPlayer.this.stop();
        }

        public void positionChanged(int position) {
            LogUtil.d(TAG, "Position Changed event received: " + position);
        ...
        public void durationChanged(int duration) {
            LogUtil.d(TAG, "Duration Changed event received: " + duration);
            synchronized (ZxtMediaPlayer.this) {
